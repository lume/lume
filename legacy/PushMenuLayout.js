...
exports.default = exports.PushMenuLayout = void 0;
...
var _TouchSync = _interopRequireDefault(require("famous/src/inputs/TouchSync"));

var _GenericSync = _interopRequireDefault(require("famous/src/inputs/GenericSync"));
...
var _callAfter = _interopRequireDefault(require("army-knife/callAfter"));
 ...
 * @extends Molecule
 */
class PushMenuLayout extends _Molecule.default {
  /**
  ...
  constructor(initialOptions) {
    super(initialOptions); // Add default values for this PushMenuLayout
    ...
    (0, _utils.simpleExtend)(this._.defaultOptions, {
      ...
      menuWidth: 200,
      menuHintSize: 10,
      // the amount of the menu that is visible before opening the menu.
      pushAreaWidth: 40,
      // the area on the screen edge that the user can touch and drag to push out the menu.
      animationDuration: 1000,
      animationType: 'foldDown',
      // options: foldDown moveBack
    ...
    this.contentWidth = document.body.clientWidth - this.options.menuHintSize; // Changing these values outside of an instance of PushMenuLayout might
    ...
    _GenericSync.default.register({
      touch: _TouchSync.default
    });
  ...
  _createComponents() {
    var layout = this;
    this.touchSync = new _GenericSync.default(['touch']);
    this.alignment = this.options.menuSide == 'left' ? 0 : 1;
    this.animationTransition = new _Transitionable.default(0);
    this.mainMol = new _Molecule.default();
    this.menuMol = new _Molecule.default({
    ...
    this.menuMol.transform = function () {
      ...
      switch (layout.options.animationType) {
        case 'foldDown':
          ...
          this.oldTransform.setTranslateX(layout.options.menuSide == 'left' ? currentPosition * (layout.options.menuWidth - layout.options.menuHintSize)
          /*range*/
          - (layout.options.menuWidth - layout.options.menuHintSize)
          /*offset*/
          : currentPosition * -(layout.options.menuWidth - layout.options.menuHintSize)
          /*range*/
          + (layout.options.menuWidth - layout.options.menuHintSize)
          /*offset*/
          );
    ...
    }.bind(this.menuMol); // contains the user's menu content.


    this.menuContentMol = new _Molecule.default();
    this.contentMol = new _Molecule.default({
    ...
    this.contentMol.transform = function () {
      ...
      switch (layout.options.animationType) {
        ...
        case 'moveBack':
          var depth = 100; // XXX: this is depending on my modifications for TransitionableTransform.
    ...
    this.menuTouchPlane = new _Plane.default({
    ...
    this.updateStyles = function () {
      ...
      this.fadeStylesheet = _jss.default.createStyleSheet(styles);
      this.fadeStylesheet.attach();
    };

    if (this.options.fade) {
      this.updateStyles();
      this.fadePlane = new _Plane.default({
        size: [undefined, undefined],
        classes: [// TODO: switch to jss namespace.
    ...
    this.mainMol.add(this.menuMol); // TODO: Also create and add a background plane for the menu area so it will catch events that might fall through the menu content.
  }
  /**
  ...
  _initializeEvents() {
    ...
    this.options.handler.on('update', function (event) {
      // update == drag
      this.isBeingDragged = true; // stop the current transitions if any, along with the current callback if any.

      this._haltAnimation(true);

      var currentPosition = this.animationTransition.get(); // TODO: handle the right-side menu.
    ...
    }.bind(this)); // TODO v0.1.0: Use a SizeAwareView instead of relying on the body, since we
  ...
  setMenu(node) {
    ...
    if (node instanceof _Molecule.default) {
  ...
  _animate(targetState, callback) {
    ...
    function setupCallback(numberOfTransitions) {
      ...
      _callback = (0, _callAfter.default)(numberOfTransitions, function () {
  ...
  render() {
    ...
    if (this.options.blur && this.options.fade && this.options.animationType == 'moveBack') {
      ...
      let filter = {
        ...
        filter: 'blur(' + momentaryBlur + 'px)' // TODO TODO TODO v0.1.0: Make fadePlane a sibling with menu and
      ...
      if (this.contentMol._child[1].get() instanceof _Surface.default) {
        this.contentMol.get().setProperties(filter);
      } else if (this.contentMol._child[1] instanceof _Plane.default) {
