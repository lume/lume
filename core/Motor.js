...
exports.default = exports.Motor = void 0;
...
class _Motor {
  constructor() {
    ...
    this.__numberOfTasks = 0;
    this.__allRenderTasks = [];
    this.__nodesToUpdate = [];
    this.__modifiedScenes = []; // A set of nodes that are the root nodes of subtrees where all nodes
    // in each subtree need to have their world matrices updated.

    this.__treesToUpdate = []; // default to requestAnimationFrame for regular non-VR/AR scenes.

    this.__requestFrame = window.requestAnimationFrame.bind(window); // TODO TS rAF signature
  }
  /**
  ...
  addRenderTask(fn) {
    ...
    this.__numberOfTasks += 1; // If the render loop isn't started, start it.
  ...
  setNodeToBeRendered(node) {
    ...
    this.__nodesToUpdate.push(node); // noop if the loop's already started
  ...
  async __startAnimationLoop() {
    ...
    while (this.__loopStarted) {
      ...
      this.__runRenderTasks(timestamp); // wait for the next microtask before continuing so that SkateJS
      ...
      this.__renderNodes(timestamp); // If no tasks are left, stop the animation loop.
  ...
  __renderNodes(timestamp) {
    ...
    for (let i = 0, l = this.__nodesToUpdate.length; i < l; i += 1) {
      const node = this.__nodesToUpdate[i]; // @ts-ignore: call protected method

      node._render( //
      timestamp); // if there is no ancestor of the current node that should be
      // rendered, then the current node is a root node of a subtree
      // that needs to be updated


      if ( // @ts-ignore: call protected method
      !node._getNearestAncestorThatShouldBeRendered() && !this.__treesToUpdate.includes(node)) {
    ...
    for (let i = 0, l = treesToUpdate.length; i < l; i += 1) {
      treesToUpdate[i] // @ts-ignore: call protected method
    ...
    for (let i = 0, l = nodesToUpdate.length; i < l; i += 1) {
      // prettier-ignore
      nodesToUpdate[i] // @ts-ignore: access protected property
