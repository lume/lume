...
exports.AutoLayout = exports.AutoLayoutNode = exports.default = void 0;

var AutoLayout = _interopRequireWildcard(require("autolayout"));
 ...
 * description.
 */
class AutoLayoutNode extends _Node.default {
  /**
   * Constructor
   *
  ...
  reflowLayout() {
    if (!this._reflowLayout) {
      ...
      _Motor.default.once(() => this.emit('reflow')); // PORTED
  ...
  setVisualFormat(visualFormat, parseOptions) {
    ...
    this._metaInfo = AutoLayout.VisualFormat.parseMetaInfo(visualFormat); // @ts-ignore
  ...
  _setViewPortSize(size, vp) {
    size = [vp.width !== undefined && vp.width !== true ? vp.width : Math.max(Math.min(size[0], vp['max-width'] || size[0]), vp['min-width'] || 0), vp.height !== undefined && vp.height !== true ? vp.height : Math.max(Math.min(size[1]
    /*TODO no !*/
    , vp['max-height'] || size[1]), vp['min-height'] || 0)];
    ...
    if (vp.width === true && vp.height === true) {
      ...
      size[0] = this._autoLayoutView.fittingWidth; // TODO ASPECT RATIO?
    } else if (vp.height === true) {
      this._autoLayoutView.setSize(size[0], undefined);

      size[1] = this._autoLayoutView.fittingHeight; // TODO ASPECT RATIO?
    } else {
  ...
  _updateNode(node, subView, x, y, widths, heights) {
    ...
    widths && widths[key] === true ? 'proportional' : 'literal', // @ts-ignore: TODO, key is not defined from anywhere, but it was working???
    heights && heights[key] === true ? 'proportional' : 'literal'];
    node.size = [// PORTED
    // @ts-ignore: TODO, key is not defined from anywhere, but it was working???
    widths && widths[key] === true ? 1 : subView.width, // @ts-ignore: TODO, key is not defined from anywhere, but it was working???
    heights && heights[key] === true ? 1 : subView.height];
    node.position = [// PORTED
    x + subView.left, y + subView.top, subView.zIndex * 5];
  }

  _checkNodes() {
    const subViews = this._autoLayoutView.subViews;
    const subViewKeys = Object.keys(subViews);
    const _idToNode = this._idToNode; // if a node is not found for a subview key, see if exists in this's DOM children by className
  ...
  _showOrHideNodes() {
    ...
    const subViewKeys = Object.keys(subViews);
    const _idToNode = this._idToNode;
    const nodeIds = Object.keys(_idToNode); // hide the child nodes that are should not be visible for the current subview.

    for (const id of nodeIds) {
      if (subViewKeys.includes(id)) _idToNode[id].visible = true;else _idToNode[id].visible = false;
    }
  }

}

exports.AutoLayoutNode = exports.default = AutoLayoutNode;
AutoLayoutNode.defaultElementName = 'i-autolayout-node';
AutoLayoutNode.DEFAULT_PARSE_OPTIONS = {
  extended: true,
  strict: false
};
AutoLayoutNode.props = _objectSpread({}, _Node.default.props || {}, {
  visualFormat: String
});