...
exports.default = exports.WebComponent = void 0;
...
var _WithChildren = _interopRequireDefault(require("./WithChildren"));
...
if (typeof window.HTMLElement != 'function') {
  ...
  _HTMLElement.prototype = window.HTMLElement.prototype(window).HTMLElement = _HTMLElement;
}
...
function WebComponentMixin(Base) {
  Base = Base || HTMLElement; // XXX: In the future, possibly check for Element if other things besides
  ...
  class WebComponent extends _WithChildren.default.mixin(_DefaultBehaviors.default.mixin((0, _lowclass.Constructor)(Base))) {
    constructor(...args) {
      super(...args);
      this.__style = null;
      this.__initialized = false;
      this.__initialAttributeChange = false; // Throw an error if no Custom Elements v1 API exists.
    ...
    async disconnectedCallback() {
      if (super.disconnectedCallback) super.disconnectedCallback(); // Deferr to the next tick before cleaning up in case the
    ...
    _init() {
      if (!this.__style) this.__style = this.__createStyles(); // fire this.attributeChangedCallback in case some attributes have
    ...
    _deinit() {// Nothing at the moment, but subclasses can extend this to add
      // deintialization logic.
    }

    __createStyles() {
      const rule = _jss.default.createRule(this.getStyles());
...
const WebComponent = (0, _lowclass.Mixin)(WebComponentMixin, HTMLElement);
