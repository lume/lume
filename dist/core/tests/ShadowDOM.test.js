...
describe('ShadowDOM support', () => {
    ...
    it('composes outer tree Nodes to a ShadowRoot Scene via slotted slot', async () => {
        const node = html `<lume-element3d slot="scene">hello</lume-element3d>`;
    ...
    it('does not compose Nodes that are not distributed to a slot of a Node in a ShadowRoot', async () => {
        const node = html `<lume-element3d></lume-element3d>`;
        container.append(node);
        const scene = html `
    ...
    it('composes outer tree Nodes to a ShadowRoot Scene via slotted slot', async () => {
        const node = html `<lume-element3d slot="middle">hello</lume-element3d>`;
        container.append(node);
        const middle = html `
			<div>
    ...
    it("composes default content of a slot to the slot's parent if the slot has no nodes distributed to it", async () => {
        const node = html `<lume-element3d slot="none">hello</lume-element3d>`;
        container.append(node);
        const middle = html `
			<div>
    ...
    it('composes a distributed node of a ShadowRoot child slot to the shadow host when the shadow host has a slot assigned to the ShadowRoot child slot', async () => {
        const node = html `<lume-element3d slot="node">hello</lume-element3d>`;
        container.append(node);
        const middle = html `
    ...
    it('tracks direct vs terminal slottedParent through a three-layer forwarded slot chain', async () => {
        ...
        const trackCallbacks = (el) => {
            const log = [];
            const origComposed = el.composedCallback;
            const origUncomposed = el.uncomposedCallback;
            const origChildComposed = el.childComposedCallback;
            const origChildUncomposed = el.childUncomposedCallback;
            el.composedCallback = function (parent, type) {
                log.push({ event: 'composed', other: parent, compositionType: type });
                return origComposed?.call(this, parent, type);
            };
            el.uncomposedCallback = function (parent, type) {
                log.push({ event: 'uncomposed', other: parent, compositionType: type });
                return origUncomposed?.call(this, parent, type);
            };
            el.childComposedCallback = function (child, type) {
                log.push({ event: 'childComposed', other: child, compositionType: type });
                return origChildComposed?.call(this, child, type);
            };
            el.childUncomposedCallback = function (child, type) {
        ...
        console.log('[3L-TEST] B.__composedParent:', B.__composedParent?.tagName);
        ...
        console.log('[3L-TEST] E === B.__composedParent:', E === B.__composedParent);
