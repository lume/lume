...
export function CompositionTracker(Base) {
    ...
    return class CompositionTracker extends ChildTracker(Constructor(Base)) {
         ...
         */
        get shadowRootChildren() {
            if (!this.exposedShadowRoot)
                return [];
            return Array.from(this.exposedShadowRoot.children).filter((n) => isAnyCompositionTracker(n));
        }
        /**
         * Elements that are slotted to a slot that is child of a ShadowRoot of
         * this element.
         */
        get shadowRootSlottedChildren() {
            const result = [];
            for (const child of Array.from(this.exposedShadowRoot?.children || [])) {
                if (child instanceof HTMLSlotElement && !child.assignedSlot) {
                    for (const slotted of child.assignedElements({ flatten: true })) {
                        if (isAnyCompositionTracker(slotted))
                            result.push(slotted);
                    }
                }
            }
        ...
        get composedChildren() {
            ...
            if (this.exposedShadowRoot) {
                ...
                return [
                    ...
                    ...Array.from(this.children).filter((n) => isAnyCompositionTracker(n)),
                ];
            }
        }
        /**
        ...
        /** @private */
        __lastComposedParent = null;
        /** @private */
        __lastCompositionType = 'actual';
        /**
        ...
        /** @private */
        __triggerChildUncomposedCallback(parent, child, compositionType) {
            // If we detected the discrepancy, return, the slotchange handler
            // will rerun this again in correct ordering.
            if (child.__discrepancy)
                return;
        ...
        __handleSlottedChildren(slot) {
            ...
            for (let l = removed.length, i = 0; i < l; i += 1) {
                ...
                if (!isAnyCompositionTracker(removedNode))
                    continue;
                ...
                if (!isForwarded) {
                    ...
                    if (this.terminalSlottedChildren) {
                        ...
                        if (!this.terminalSlottedChildren.size)
                            this.terminalSlottedChildren = null;
                    }
            ...
            for (let l = added.length, i = 0; i < l; i += 1) {
                ...
                if (!isAnyCompositionTracker(addedNode))
                    continue;
                // --- Direct slotted* — the nearest slot-owning parent ---
                ...
                if (!addedNode.slottedParent) {
                    ...
                    if (isUpgraded)
                        ...
                        customElements.whenDefined(addedNode.tagName.toLowerCase()).then(trigger);
                }
                else {
                ...
                if (!isForwarded) {
                    ...
                    if (terminalParent) {
                        ...
                        if (children) {
                            ...
                            if (!children.size)
                                terminalParent.terminalSlottedChildren = null;
                        }
                    }
                    // The node is now slotted to `this` element.
                    addedNode.terminalSlottedParent = this;
                    if (!this.terminalSlottedChildren)
                        this.terminalSlottedChildren = new Set();
                    ...
                    if (addedNode.__composedParent)
                        addedNode.__discrepancy = true;
                    console.log('[TERMINAL-ADD] this:', this.tagName, 'added:', addedNode.tagName, '__CP before:', addedNode.__composedParent?.tagName);
                    ...
                    addedNode.__lastComposedParent = this;
                    addedNode.__lastCompositionType = 'terminal-slot';
                    console.log('[TERMINAL-ADD] SET __CP of', addedNode.tagName, 'to', this.tagName, '__CP now:', addedNode.__composedParent?.tagName);
                    const trigger = () => {
...
export function isAnyCompositionTracker(o) {
    return o[isInstance];
}
