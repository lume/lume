...
let GltfModelBehavior = (() => {
    let _classDecorators = [element('gltf-model', autoDefineElements)];
    ...
    var GltfModelBehavior = class extends _classSuper {
        ...
        static {
            ...
            _src_decorators = [attribute];
            _dracoDecoder_decorators = [stringAttribute];
        ...
        #loadModel() {
            ...
            this.loader.load(src, model => version == this.#version && this.#setModel(model), progress => version == this.#version && this.composedParent.emit(Events.PROGRESS, progress), error => version == this.#version && this.#onError(error));
        }
        #onError(error) {
            const message = `Failed to load ${this.composedParent.tagName.toLowerCase()} with src "${this.src}" and dracoDecoder "${this.dracoDecoder}". See the following error.`;
        ...
        #setModel(model) {
            ...
            this.composedParent.three.add(model.scene);
            this.composedParent.emit(Events.MODEL_LOAD, { format: 'gltf', model });
