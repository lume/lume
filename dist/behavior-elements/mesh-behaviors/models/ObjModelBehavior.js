...
let ObjModelBehavior = (() => {
    ...
    var ObjModelBehavior = class extends _classSuper {
        ...
        #loadObj(version, hasMtl) {
            this.objLoader.load(this.obj, model => version == this.#version && this.#setModel(model, hasMtl), progress => version === this.#version && this.composedParent.emit(Events.PROGRESS, progress), error => version === this.#version && this.#onError(error));
        }
        #onError(error) {
            const message = `Failed to load ${this.composedParent.tagName.toLowerCase()} with obj value "${this.obj}" and mtl value "${this.mtl}". See the following error.`;
        ...
        #setModel(model, hasMtl) {
            ...
            if (!hasMtl) {
                ...
                for (const child of this.composedParent.children) {
                    if (!(child instanceof MaterialBehavior))
                        continue;
