...
let ObjModelBehavior = (() => {
    let _classDecorators = [element('obj-model', autoDefineElements)];
    ...
    var ObjModelBehavior = class extends _classSuper {
        ...
        static {
            ...
            _obj_decorators = [stringAttribute];
            _mtl_decorators = [stringAttribute];
        ...
        #loadObj(version, hasMtl) {
            this.objLoader.load(this.obj, model => version == this.#version && this.#setModel(model, hasMtl), progress => version === this.#version && this.parentElement.emit(Events.PROGRESS, progress), error => version === this.#version && this.#onError(error));
        }
        #onError(error) {
            const message = `Failed to load ${this.parentElement.tagName.toLowerCase()} with obj value "${this.obj}" and mtl value "${this.mtl}". See the following error.`;
                ...
                for (let i = 0; i < children.length; i++) {
                    ...
                    if (child instanceof HTMLElement &&
                        (child.tagName.toLowerCase().includes('material') ||
                            ...
                            child instanceof globalThis.MaterialBehavior)) {
