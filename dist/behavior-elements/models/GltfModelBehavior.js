...
let GltfModelBehavior = (() => {
    ...
    var GltfModelBehavior = class extends _classSuper {
        ...
        #loadModel() {
            ...
            this.loader.load(src, model => version === this.#version && this.#setModel(model), progress => version === this.#version && this.parentElement?.emit(Events.PROGRESS, progress), error => version === this.#version && this.#onError(error));
        }
        #onError(error) {
            const message = `Failed to load ${this.parentElement?.tagName.toLowerCase()} with src "${this.src}". See the following error.`;
...
function getDracoLoader(decoderPath) {
...
function disposeDracoLoader(decoderPath) {
