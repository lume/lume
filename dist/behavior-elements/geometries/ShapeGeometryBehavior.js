...
import { attribute, booleanAttribute, numberAttribute, stringAttribute, element } from '@lume/element';
...
let ShapeGeometryBehavior = (() => {
    let _classDecorators = [element('shape-geometry', autoDefineElements)];
    ...
    var ShapeGeometryBehavior = class extends _classSuper {
        ...
        static {
            ...
            _get_shape_decorators = [attribute];
            _set_shape_decorators = [attribute];
            _curveSegments_decorators = [numberAttribute];
            _bevel_decorators = [booleanAttribute];
            _bevelSegments_decorators = [numberAttribute];
            _bevelThickness_decorators = [numberAttribute];
            _centerGeometry_decorators = [booleanAttribute];
        ...
        _createComponent() {
            let geometry;
            if (this.parentElement.calculatedSize.z === 0) {
                ...
                geometry = new ExtrudeGeometry(this.shape, {
                    ...
                    depth: this.parentElement.calculatedSize.z,
                });
            }
            ...
            const scaleX = shapeSizeX / this.parentElement.calculatedSize.x;
            const scaleY = shapeSizeY / this.parentElement.calculatedSize.y;
            if (this.fitment === 'fill')
                return geometry.scale(1 / scaleX, 1 / scaleY, 1);
            const shapeAspect = shapeSizeX / shapeSizeY;
            const sizeAspect = this.parentElement.calculatedSize.x / this.parentElement.calculatedSize.y;
            if (this.fitment === 'contain') {
                ...
                if (!(shapeSizeX <= this.parentElement.calculatedSize.x && shapeSizeY <= this.parentElement.calculatedSize.y)) {
                    // tall
                    if (shapeAspect < sizeAspect)
                        geometry.scale(1 / scaleY, 1 / scaleY, 1);
                    // wide (or equal)
