...
var rafLoop = {
   ...
   */
  blockFor: function blockFor(time) {
   ...
   */
  onNextTick: function onNextTick(func, context, data) {
  ...
  _runTick: function _runTick(timestamp) {
    var current,
        data,
        completed = true;
    ...
    if (rafLoop._queue.head) {
      ...
      for (current = rafLoop._queue.head; current; current = current.next) {
        ...
        if (!rafLoop.timeOk() && current.next) {
          if (++rafLoop._exceedCount === 30) log.debug("Didn't complete all updates for 30 frames in a row!");
...
var SinglyLinkedList = (function (_SinglyLinkedList) {
  ...
  _SinglyLinkedListWrapper.toString = function () {
    return _SinglyLinkedList.toString();
  };

  return _SinglyLinkedListWrapper;
})(function () {
  if (!(this instanceof SinglyLinkedList)) {
    // called without 'new'
    var list = listPool && listPool.shift();
...
SinglyLinkedList.prototype.getNext = function () {
  ...
  if (this.current === undefined) this.current = this.head;
...
SinglyLinkedList.prototype.recycle = function () {
  ...
  if (this.head) {
    if (elementPool.tail) elementPool.tail.next = this.head;else elementPool.head = this.head;
...
var log = {
  ...
  showLevel: function showLevel(level) {
    return this.levels.slice(0, this.levels.indexOf(this.level) + 1) // all current levels
    .indexOf(level) !== -1;
  },

  out: function out(level) {
    ...
    if (!this.showLevel(level)) {
      return;
    }var prepend = "%c [famin] " + level + ": ";

    if (typeof args[0] === "string") args[0] = prepend + args[0];else args.splice(0, 0, prepend);
...
for (var x = 0; x < vendors.length && !raf.requestAnimationFrame; ++x) {
    raf.requestAnimationFrame = raf[vendors[x] + "RequestAnimationFrame"] && raf[vendors[x] + "RequestAnimationFrame"].bind(window);
    raf.cancelAnimationFrame = raf[vendors[x] + "CancelAnimationFrame"] && raf[vendors[x] + "CancelAnimationFrame"].bind(window) || raf[vendors[x] + "CancelRequestAnimationFrame"] && raf[vendors[x] + "CancelRequestAnimationFrame"].bind(window);
}

if (!raf.requestAnimationFrame) raf.requestAnimationFrame = function (callback, element) {
...
if (!raf.cancelAnimationFrame) raf.cancelAnimationFrame = function (id) {
    clearTimeout(id);
};
