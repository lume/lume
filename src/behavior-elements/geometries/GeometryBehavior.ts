...
export abstract class GeometryBehavior extends GeometryOrMaterialBehavior {
	...
	get size() {
		return this.parentElement?.size
	}
	set size(val) {
		if (this.parentElement) this.parentElement.size = val
	}

	get sizeMode() {
		return this.parentElement?.sizeMode
	}
	set sizeMode(val) {
		if (this.parentElement) this.parentElement.sizeMode = val
	}
