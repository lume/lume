...
import {ProjectedMaterial as ThreeProjectedMaterial} from '@lume/three-projected-material/dist/ProjectedMaterial.js'
...
export type ProjectedMaterialAttributes = PhysicalMaterialAttributes | 'textureProjectors'

/**
 * @class ProjectedMaterial
 *
 * Element: `<lume-projected-material>`
 *
 ...
 * associating it with the `texture-projectors` attribute on a
 * `<lume-projected-material>`:
 *
 * ```html
 * <!-- Define a texture projector somewhere. -->
 * <lume-texture-projector src="path/to/image.jpg" class="some-projector" position="200 0 200" rotation="0 45 0"></lume-texture-projector>
 *
 * <!-- Use the projected-material on the mesh, and associate it with the projector: -->
 * <lume-box size="100 100 100">
 *   <lume-projected-material  texture-projectors=".some-projector"></lume-projected-material>
 * </lume-box>
 * ```
 *
 ...
 */
export
@element('lume-projected-material', autoDefineElements)
class ProjectedMaterial extends PhysicalMaterial {
	 ...
	 * ```html
	 * <lume-box>
	 *   <lume-projected-material  texture-projectors=".foo, .bar, #baz"></lume-projected-material>
	 * </lume-box>
	 * ```
	 *
	 ...
	 * ```
	 *
	 * Texture projectors that are not in the flat tree (i.e. not
	...
	override _createComponent() {
		...
		return new ThreeProjectedMaterial()
	}
	...
	protected override _parentDefinedEffect(parent: NonNullable<this['composedParent']> = this.composedParent!) {
		...
		// TODO replace with a mutations signal
		...
		for (const root of upwardRoots(parent)) observer.observe(root, {childList: true, subtree: true})
		...
		this.createEffect(() => {
			...
			createEffect(() => {
				...
				for (const value of array) {
					if (typeof value !== 'string') {
						// Consider only elements that participate in rendering (i.e. that are in the flat tree)
					...
					for (const el of querySelectorUpward(parent, value)) {
						if (!el) continue
			...
			createEffect(() => {
				...
				// triggered when parent has its world matrix updated.
				parent.version
				mat.project(three as any, false)
			})
			...
			createEffect(() => {
				...
				createEffect(() => {
					...
					tex.version
					mat.project(three as any, false)
					parent.needsUpdate() // The texture element updated, so make sure parent does too.
				})
			})
		})
	}
...
declare module 'solid-js' {
	namespace JSX {
		interface IntrinsicElements {
			'lume-projected-material': ElementAttributes<ProjectedMaterial, ProjectedMaterialAttributes>
		}
	}
}
...
declare global {
	interface HTMLElementTagNameMap {
		'lume-projected-material': ProjectedMaterial
	}
}
