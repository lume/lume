...
import {materialBehaviorChildren} from '../utils.js'
 ...
 */
export
@element('lume-clipper', autoDefineElements)
class Clipper extends MeshBehaviorEl {
	/**
	...
	@stringAttribute get clipPlanes(): Array<ClipPlane> {
	...
	@stringAttribute set clipPlanes(value: string | Array<ClipPlane | string>) {
		...
		for (const v of array) {
			...
			let root = this.composedParent!.getRootNode() as Document | ShadowRoot | null
	...
	protected override _parentDefinedEffect(parent: NonNullable<this['composedParent']> = this.composedParent!) {
		...
		if (!refCount) parent.scene.localClipping = true
		refCount++
		...
		this.#observer.observe(parent.getRootNode(), {childList: true, subtree: true})

		createEffect(() => {
			const {clipPlanes, clipDisabled} = this

			if (!clipPlanes.length || clipDisabled) return

			const skip = this
			const materialBehaviors = materialBehaviorChildren(parent, skip)

			createEffect(() => {
				const mat = materialBehaviors()[0]?.meshComponent
				if (!mat) return
				...
				createEffect(() => {
					...
					mat.clippingPlanes = []
					for (const plane of clipPlanes) mat.clippingPlanes.push(flipClip ? plane.threeInverseClip : plane.threeClip)

					createEffect(() => {
						mat.clipIntersection = this.clipIntersection
						mat.clipShadows = this.clipShadows
					})
		...
		onCleanup(() => {
			...
			refCount--
			if (!refCount) this.#lastScene!.localClipping = false
			this.#lastScene = null
		})
	}
}
