...
describe('Mixin', () => {
    ...
    it('When Symbol is not supported, instanceof does not work', () => {
        ...
        function test() {
            ...
            console.log('one is One?',  one instanceof One )

            expect( one instanceof One ).toBe( true )

            console.log('one is Ipsum?',  one instanceof Ipsum )
        ...
        window.Symbol = () => Math.random()

        console.log(' ################ ', window.Symbol, window.Symbol)
        test()

        window.Symbol = void 0
    ...
    it('if a class already has its own Symbol.hasInstance method, we do not override it', () => {
        ...
        const FooMixin = HasInstance(Base => {
            ...
            Object.defineProperty(Class, window.Symbol.hasInstance, { value: fn })
        ...
        expect( Foo[window.Symbol.hasInstance] ).toBe( fn )

        const BarMixin = HasInstance(Base => class Bar extends Base {})

        const Bar = BarMixin(class{})

        expect( Bar[window.Symbol.hasInstance] ).not.toBe( fn )
    })
